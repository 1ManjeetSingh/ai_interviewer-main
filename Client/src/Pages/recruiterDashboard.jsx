...
const RecruiterDashboard = () => {
  ...
  const getTranscriptData = async () => {
    try {
      ...
      if (data?.analysis.data_collection_results?.['Job title']?.value && data?.analysis.data_collection_results?.['Designation']?.value) {
        handleGenerate(data?.analysis.data_collection_results?.['Job title']?.value, data?.analysis.data_collection_results?.['Designation']?.value);
      }
  ...
  const roundsList = [{ label: 'Job Posted', key: 'jobPosted' },
  ...
  const textToShow = {
    ...
    applicantsApplied: `Exciting news! ${jobCards[Object.keys(jobCards)[activeIndex]].applicantsApplied} eager candidates are vying for this roleAI is analyzing their potential.`,
  ...
  useEffect(() => {
    if (roundsWithGraph.includes(jobCards[Object.keys(jobCards)[activeIndex]].isActive)) {
      ...
      <div className="NavBar w-full mx-[auto] h-[8vh] min-h-[42px] px-8 bg-white border border-[#D2D2D2] backdrop-blur-[220px] flex justify-between items-center hover:cursor-pointer">
        ...
        <div className="Frame1000008205 flex justify-start items-center w-fit gap-[1vw] bg-none" >
          ...
          <div className="Ai w-[4.5vw] min-w-[42px] bg-none" >
            <img onClick={toggleLiylaStatus} className="Layila h-[7.5vh] bg-none rounded-full hover:transform hover:scale-[1.08] hover:transition-transform hover:duration-300" src={image4} />
          </div>
        </div>
      </div>
        ...
        <div className="flex justify-center mt-[3vh] gap-12 w-[100%]">
          ...
          >
            <div className="self-stretch justify-between items-start inline-flex">
              <div className="w-full flex-col justify-start items-start gap-[1vh] inline-flex pl-2">
                ...
                <div className="self-stretch flex-col justify-start items-start gap-[0.5vh] flex">
                  ...
                  <div className="text-center text-[#6f6f6f] text-2xl font-normal font-['SF UI Text'] leading-[2.5vh]">
                    Posted On : {jobCards[Object.keys(jobCards)[activeIndex]].postedOn}
                  </div>
                </div>
              </div>
              <div className="w-fit px-[1.5vh] mt-1 bg-neutral-100 rounded-[2.5vw] shadow-[0px_0px_4px_0px_rgba(0,0,0,0.25)] justify-start items-center gap-[1vw] flex">
                <Select
                  ...
                  isDisabled={!roundsWithGraph.includes(jobCards[Object.keys(jobCards)[activeIndex]].isActive)}
                />
            ...
            <div className="w-[100%] h-[350px] py-[2vh] bg-none rounded-[24px] shadow-[0px_0.5vw_1.5vw_0px_rgba(0,0,0,0.25)] border border-[#dcffff] flex-col justify-center items-center flex shrink">
              ...
              {roundsWithGraph.includes(jobCards[Object.keys(jobCards)[activeIndex]].isActive) ?
                (<ResponsiveLine
                  data={lineData}
                ...
                />) : (<div className="justify-start items-center gap-[1vw] inline-flex px-8">
                  ...
                  >
                    {textToShow[jobCards[Object.keys(jobCards)[activeIndex]].isActive]}
                  </div>
                  ...
                  <SwiperSlide key={index}>
                    ...
                    <div className="px-4 pb-8 flex-col mb-auto justify-start items-start gap-10 inline-flex mt-14 relative">
                      {roundsList.map(({ label, key }) => {
                        const isActive = jobCards[Object.keys(jobCards)[activeIndex]].isActive === key;
                        const status =
                          job?.progress?.[key] !== 0 ? 'completed' : isActive ? 'active' : 'pending';
                        ...
                        return (
                          <div key={key} className="self-stretch justify-start items-center gap-6 inline-flex">
                            <div
                              ...
                              style={{
                                background: jobCards[Object.keys(jobCards)[activeIndex]].isActive === key
                            ...
                            />
                            <div
                              className={`text-center text-xl font-['SF UI Text'] leading-tight ${status === 'completed' ? 'text-[#7D7DA4]' : (status === 'pending' ? 'text-[#C3C3EA]' : '')}`}
                              style={
                                status === 'active' ?
                                  {
                                    backgroundImage: gradients[gradientIndex],
                            ...
                            >
                              {(key === 'applicantsApplied' && job?.progress?.applicantsApplied !== 0) ? `${job?.progress?.applicantsApplied}` : ''}&nbsp;{label}
                            </div>
                          </div>
                        );
                      })}
